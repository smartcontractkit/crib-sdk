...
type Props struct {
	...
	Rules       []*k8s.PolicyRule `validate:"required,gt=0,dive,required"`
}
...
// New creates a new Kubernetes Role scalar component.
func New(ctx context.Context, props crib.Props) (*Role, error) {
	...
	chart := cdk8s.NewChart(parent, crib.ResourceID("sdk.scalar.RoleV1", props), nil)
	...
	role := k8s.NewKubeRole(chart, resourceID, &k8s.KubeRoleProps{
		Metadata: metadata,
		Rules:    dry.ToPtr(scalarProps.Rules),
	})
